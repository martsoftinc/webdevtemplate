...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-slide">
  ...
  <div class="section-card">
    ...
    <div class="flex items-center gap-4 mb-6 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60">
      <div class="avatar-ring" id="avatarInitials">{{ strtoupper(substr(Auth::user()->name ?? 'AU', 0, 2)) }}</div>
      <div>
        <div class="font-syne font-bold text-slate-900 dark:text-white text-lg" id="avatarName">{{ Auth::user()->name ?? 'Admin User' }}</div>
        <div class="text-sm text-slate-500 dark:text-slate-400">{{ Auth::user()->role ?? 'Super Admin' }}</div>
      </div>
    </div>
    ...
    <form id="profileForm" action="" method="POST">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="sm:col-span-2">
          ...
          <input type="email" id="email" name="email" class="form-input"
            ...
            placeholder="e.g. admin@bedncy.com" autocomplete="email" />
        </div>
      </div>
      ...
      <div class="flex items-center gap-4 mt-6">
        ...
        <div class="save-success" id="profileSuccess">
          <svg class="w-4 h-4 check-pop" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
  ...
  <div class="section-card">
    ...
    <form id="passwordForm" action="#" method="POST">
      ...
      <div class="flex items-center gap-4 mt-6">
        <button type="submit" class="btn-primary" style="background:#f59e0b;" onmouseover="this.style.background='#d97706'" onmouseout="this.style.background='#f59e0b'">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z"/>
          </svg>
          Update Password
        </button>
        <div class="save-success" id="passwordSuccess">
          <svg class="w-4 h-4 check-pop" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/>
          </svg>
          Updated!
  ...
  <div class="section-card">
    ...
    <div class="flex items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60">
      <div class="flex items-start gap-3">
        ...
        <div>
          <div class="flex items-center gap-2">
            ...
            <span class="badge-2fa off" id="twoFaBadge">Off</span>
          </div>
      ...
      {{-- Toggle --}}
      <div role="switch" aria-checked="false" id="twoFaToggle" tabindex="0"
           class="toggle-track" title="Toggle 2FA"
           onclick="toggle2FA()" onkeydown="if(event.key==='Enter'||event.key===' ')toggle2FA()">
        <div class="toggle-knob"></div>
      </div>
    </div>

    {{-- 2FA info box, shown when enabled --}}
    <div id="twoFaInfo" class="hidden mt-4 p-4 rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/40">
...
<script>
  ...
  /* ── 2FA Toggle ── */
  let twoFaEnabled = {{ Auth::user()->two_factor_enabled ?? 'false' ? 'true' : 'false' }};
  ...
  /* Demo: intercept forms for UI feedback (remove in real app) */
  document.getElementById('profileForm')?.addEventListener('submit', function(e) {
    e.preventDefault(); // remove this line when wired to Laravel
